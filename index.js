...
setInterval(() => {
    ...
    client.channels.cache.get(config.serverStats.humans).setName(`❤️| Citoyen : ${humans.size}`)
    client.channels.cache.get(config.serverStats.bots).setName(`🐶| Esclave : ${bots.size}`)
    client.channels.cache.get(config.serverStats.total).setName(`🕍| Ville : ${client.guilds.cache.first().memberCount}`)
}, 3e4)
})